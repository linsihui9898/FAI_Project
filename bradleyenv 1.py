...
class BradleyAirportEnv(gym.Env):
    def __init__(self, screen_width=800, screen_height=800):
        ...
        self.observation_space = spaces.Box(low=0, high=1, shape=(self.num_runways + self.num_taxiways + 2,),
                                            dtype=np.float32)
    ...
    def step(self, action):
        ...
        return np.array([self.traffic, self.weather] + self.runways + [self.taxiway],
    ...
    def render(self):
        ...
        print(
            f"Time Step: {self.time_step} | Traffic: {self.traffic} | Runways: {self.runways} | Taxiway: {self.taxiway} | Weather: {self.weather}")
    ...
    def update(self):
        for plane in self.planes:
            plane.move()
            if plane.is_off_screen(): # Remove out of bound planes
                env.remove_plane(plane)
